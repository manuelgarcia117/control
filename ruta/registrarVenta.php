...
$use->bindValue(16, $precinto);
$use->bindValue(17, $factura);
$use->bindValue(18, "3");
$use->bindValue(19, $placaTrailer);
$use->bindValue(20, $empresaTransportadora);
$status = $use->execute();
...
$data->idRuta = $lastId;
	if($status){
		...
		if($evidencia!=""){
        	...
        	$file = UPLOAD_DIR . 'salida.png';
        	$success = file_put_contents($file, $dataImg);
		}	
		
	} else {
		$estado = "ERROR";
		$mensaje = "Error al registrar el trayecto";
	}
}
$data->estado = $estado;
$data->codigo = $codigo;
